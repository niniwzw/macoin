	...
	*/
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %s MAC %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    printf("ReacceptWalletTransactions found spent coin %s MAC %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
bool CWallet::SelectCoinsMinConfBack(int64_t nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet) const
{
    setCoinsRet.clear();
    nValueRet = 0;
...
map<uint256, CWalletTx> mapMyWallet;
bool CWallet::SelectCoinsBack(int64_t nTargetValue, unsigned int nSpendTime, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet, const CCoinControl* coinControl, const CTxDestination& fromaddress) const
{
    vector<COutput> vecUnspent;
    AvailableCoinsForAddress(mapMyWallet, vecUnspent, fromaddress);
	//fill coins
    vector<COutput> vCoins;
    BOOST_FOREACH(const COutput& out, vecUnspent)
    {
        CTxDestination address;
        if(!ExtractDestination(out.tx->vout[out.i].scriptPubKey, address)) {
			printf("SelectCoinsBack ExtractDestination(out.tx->vout[out.i].scriptPubKey, address)\n");
            continue;
        }
        if (address == fromaddress) {
			printf("SelectCoinsBack %s\n", out.ToString().c_str());
            vCoins.push_back(out);
        }
    }
	printf("vCoins size = %"PRIszu"\n", vCoins.size());
    // coin control -> return all selected outputs (we want all selected to go into the transaction for sure)
    if (coinControl && coinControl->HasSelected())
    {
        BOOST_FOREACH(const COutput& out, vCoins)
    ...
    return (SelectCoinsMinConfBack(nTargetValue, nSpendTime, 10, 10, vCoins, setCoinsRet, nValueRet) ||
            SelectCoinsMinConfBack(nTargetValue, nSpendTime, 2, 2, vCoins, setCoinsRet, nValueRet) ||
            SelectCoinsMinConfBack(nTargetValue, nSpendTime, 1, 1, vCoins, setCoinsRet, nValueRet));
}
...
bool CWallet::CreateTransactionBack(const vector<pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl, CTransaction& txback)
{
    int64_t nValue = 0;
    ...
    wtxNew.BindWallet(this);
    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                double dPriority = 0;
                // vouts to the payees
                wtxNew.vout.push_back(CTxOut(vecSend[0].second,  txback.vout[0].scriptPubKey));
                // Choose coins to use
                ...
                // Fill vin
				//先插入找回交易
				wtxNew.vin.push_back(CTxIn(txback.GetHash(), 0));
                ...
                BOOST_FOREACH(const PAIRTYPE(const CWalletTx*,unsigned int)& coin, setCoins) {
                    total += coin.second;
                    wtxNew.vin[nIn++].scriptSig = fromscript;
                }
                // Limit size
...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl, bool isback)
{
    int64_t nValue = 0;
    BOOST_FOREACH (const PAIRTYPE(CScript, int64_t)& s, vecSend)
    {
...
bool CWallet::CreateTransactionV2(const vector<pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, bool& complete, map<uint160, CScript>& redeemScript, const CCoinControl* coinControl, bool isback)
{
    int64_t nValue = 0;
    BOOST_FOREACH (const PAIRTYPE(CScript, int64_t)& s, vecSend)
    {
...
bool CWallet::CreateTransaction(CScript scriptPubKey, int64_t nValue, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl, bool isback)
{
    vector< pair<CScript, int64_t> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    return CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, coinControl, isback);
}


bool CWallet::CreateTransactionBack(CScript scriptPubKey, int64_t nValue, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl, CTransaction& tx)
{
    vector< pair<CScript, int64_t> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    return CreateTransactionBack(vecSend, wtxNew, reservekey, nFeeRet, coinControl, tx);
}
...
string CWallet::SendMoney(CScript scriptPubKey, int64_t nValue, CWalletTx& wtxNew, bool fAskFee, bool isback)
{
    ...
    if (!CreateTransaction(scriptPubKey, nValue, wtxNew, reservekey, nFeeRequired, NULL, isback))
    {
        string strError;
...
string CWallet::SendMoneyBack(CScript scriptPubKey, int64_t nValue, CWalletTx& wtxNew, bool fAskFee, CTransaction& tx)
{
	CReserveKey reservekey(this);
    int64_t nFeeRequired;
    if (IsLocked())
    {
        string strError = _("Error: Wallet locked, unable to create transaction  ");
        printf("SendMoneyBack() : %s", strError.c_str());
        return strError;
    }
    if (fWalletUnlockStakingOnly)
    {
    ...
    if (!CreateTransactionBack(scriptPubKey, nValue, wtxNew, reservekey, nFeeRequired, NULL, tx))
    {
        string strError = _("Error: Transaction creation failed  ");
        printf("SendMoneyBack() : %s", strError.c_str());
        return strError;
    }
    if (fAskFee && !uiInterface.ThreadSafeAskFee(nFeeRequired, _("Sending...")))
        return "SendMoneyBack ABORTED";
...
string CWallet::SendMoneyToBackDestination(const CTxDestination& fromaddress, int64_t nValue, CWalletTx& wtxNew, bool fAskFee, CTransaction& tx)
{
	...
	if (!ExtractDestination(tx.vout[0].scriptPubKey, txaddress)) {
	    return _("err back transaction");
	}
	int n = 0;
	...
	if (n == 0)
	{
		return _("err back transaction and fromaddress pair");
	}
    CScript scriptPubKey;
    scriptPubKey.SetDestination(fromaddress);
    return SendMoneyBack(scriptPubKey, nValue, wtxNew, fAskFee, tx);
}
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpentCoins found lost coin %s MAC %s[%d], %s\n",
            ...
            {
                printf("FixSpentCoins found spent coin %s MAC %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), pcoin->GetHash().ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
bool IsMineTx(const CTxDestination& address, const map<uint256, CWalletTx>& mapMyWallet, const CTxOut& txout) {
	CTxDestination address2;
	if (!ExtractDestination(txout.scriptPubKey, address2)) {
		return false;
	}
	if (address == address2 && txout.nValue >= nMinimumInputValue) {
		return true;
	}
	return false;
}

bool IsMineTx(const CTxDestination& address, const map<uint256, CWalletTx>& mapMyWallet, const CTransaction& tx) {
	if (tx.IsBack())
	{
		return false;
	}
	BOOST_FOREACH(const CTxOut& txout, tx.vout) {
		if (IsMineTx(address, mapMyWallet, txout)) {
			return true;
		}
	}
	return false;
...
bool IsFromMeTx(const CTxDestination& address, const map<uint256, CWalletTx>& mapMyWallet, const CTransaction& tx) {
	if (tx.IsBack()) {
		return false;
	}
	BOOST_FOREACH(const CTxIn& txin, tx.vin) {
		if (mapMyWallet.count(txin.prevout.hash)) {
			return true;
		}
	}
	return false;
...
void MyWalletUpdateSpent(const CTxDestination& address, map<uint256, CWalletTx>& mapMyWallet, const CTransaction &tx)
{
	BOOST_FOREACH(const CTxIn& txin, tx.vin)
	{
		map<uint256, CWalletTx>::iterator mi = mapMyWallet.find(txin.prevout.hash);
		if (mi != mapMyWallet.end())
		{
			CWalletTx& wtx = (*mi).second;
			if (txin.prevout.n >= wtx.vout.size())
				printf("MyWalletUpdateSpent: bad wtx %s\n", wtx.GetHash().ToString().c_str());
			else if (!wtx.IsSpent(txin.prevout.n))
			{
				//printf("MyWalletUpdateSpent found spent coin %s MAC %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
				wtx.MarkSpent(txin.prevout.n);
			}
		}
	}
...
void AddToWalletMap(const CTransaction& tx, const CBlock* pblock, const CTxDestination& address, map<uint256, CWalletTx>& mapMyWallet)
{
    uint256 hash = tx.GetHash();
	bool fExisted = mapMyWallet.count(hash);
	if (fExisted) return;
	if (IsMineTx(address, mapMyWallet, tx) || IsFromMeTx(address, mapMyWallet, tx))
	{
		MyWalletUpdateSpent(address, mapMyWallet, tx);
		...
		mapMyWallet.insert(make_pair(hash, wtx));
	}
}
...
void AvailableCoinsForAddress(map<uint256, CWalletTx>& mapMyWallet, vector<COutput>& vCoins, const CTxDestination& address)
{
	...
	while (pindex)
	{
		...
		BOOST_FOREACH(CTransaction& tx, block.vtx)
		{
	        AddToWalletMap(tx, &block, address, mapMyWallet);
		}
		pindex = pindex->pnext;
	}
	printf("mapMyWallet.size() = %"PRIszu"\n",    mapMyWallet.size());
	for (map<uint256, CWalletTx>::const_iterator it = mapMyWallet.begin(); it != mapMyWallet.end(); ++it)
	{
		const CWalletTx* pcoin = &(*it).second;
		if (!IsFinalTx(*pcoin)) {
			printf("IsFinalTx(*pcoin) \n");
			continue;
		}
		if (pcoin->IsCoinBase() && pcoin->GetBlocksToMaturity() > 0) {
			printf("pcoin->IsCoinBase() && pcoin->GetBlocksToMaturity() > 0 \n");
			continue;
		}
		if(pcoin->IsCoinStake() && pcoin->GetBlocksToMaturity() > 0) {
			printf("pcoin->IsCoinStake() && pcoin->GetBlocksToMaturity() > 0 \n");
			continue;
		}
		int nDepth = pcoin->GetDepthInMainChain();
		if (nDepth < 0) {
		...
		for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
			if (!(pcoin->IsSpent(i)) && IsMineTx(address, mapMyWallet, pcoin->vout[i])) {
				vCoins.push_back(COutput(pcoin, i, nDepth));
			}
		}
	}
