...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new bityuan address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current bityuan address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <bityuanaddress> <account>\n"
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress <bityuanaddress> <amount> [isback] [comment] [comment-to]\n"
    ...
    string strError = pwalletMain->SendMoneyToDestination(address.Get(), nAmount, wtx, false, isback);
    if (strError != "")
...
Value sendtoaddress2(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            "sendtoaddress2 <bityuanaddress> <amount> [code]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
Value createrawtransaction2(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "createrawtransaction2 <bityuanaddress> <amount> [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <bityuanaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <bityuanaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <bityuanaddress> [minconf=1]\n"
            "Returns the total amount received by <bityuanaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <tobityuanaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
Value addbackaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 5 || params.size() > 6)
    {
        string msg = "addbackaddress <nrequired> <'[\"key\",\"key\"]'> <'[\"backkey\",\"backkey\"]'> "
            ...
            "each key is a bityuan address or hex-encoded public key\n"
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; bityuan server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
public:
    ...
    Object operator()(const CScriptID &scriptID) const {
        ...
        int nRequired;
        ExtractDestinations2(subscript, whichType, addresses, backaddresses, nRequired, vMinTime);
        ...
        BOOST_FOREACH(const CTxDestination& addr, backaddresses)
            b.push_back(CBitcoinAddress(addr).ToString());
        obj.push_back(Pair("backaddresses", b));
		Array c;
        BOOST_FOREACH(const int& mintime, vMinTime)
            c.push_back(mintime);
        obj.push_back(Pair("backmintime", c));
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <bityuanaddress>\n"
            "Return information about <bityuanaddress>.");
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <bityuanpubkey>\n"
            "Return information about <bityuanpubkey>.");
